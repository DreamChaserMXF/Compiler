...
void AssemblyMaker::DataSegment() throw() {
    ...
    for (size_t i = 0; i < stringtable_.size(); ++i) {
        assemble_buffer << "\n    _String" << i << "             db ";
        for (string::const_iterator iter = stringtable_[i].begin(); iter != stringtable_[i].end();
             ++iter) {
...
void AssemblyMaker::OtherFunction(TokenTable::const_iterator c_iter) throw() {
    ...
    assemble_buffer << "\n_" << c_iter->name_ << distance(tokentable_.begin(), c_iter)
                    << "  proc near";
    assemble_buffer << "\n    push    ebp"
                    << "\n    mov     ebp,   esp"
                    << "\n    sub     esp,   " << 4 * (var_space + temp_space);
...
// 翻译四元式
void AssemblyMaker::TranslateQuaternary(const vector<Quaternary>::const_iterator& c_iter,
                                        int para_num,
                                        int var_space,
                                        int level) throw() {
...
void AssemblyMaker::TranslateNeg(const vector<Quaternary>::const_iterator& c_iter,
                                 int para_num,
                                 int var_space,
                                 int level) throw() {
    ...
    if (c_iter->method1_ == c_iter->method3_ && c_iter->src1_ == c_iter->dst_) {
        ...
        OpGeneral(NEG, c_iter->method1_, c_iter->src1_, Quaternary::NIL_ADDRESSING, 0, para_num,
                  var_space, level);
    } else {
        // 装载源操作数至EAX
        OpRegisterGeneral(MOV, EAX, c_iter->method1_, c_iter->src1_, c_iter->method2_,
                          c_iter->offset2_, para_num, var_space, level);
        // 取反
        assemble_buffer << "\n    neg     eax";
        // 将EAX存回内存
        OpGeneralRegister(MOV, c_iter->method3_, c_iter->dst_, Quaternary::NIL_ADDRESSING, 0,
                          para_num, var_space, level);
    }
}
...
void AssemblyMaker::TranslateAdd(const vector<Quaternary>::const_iterator& c_iter,
                                 int para_num,
                                 int var_space,
                                 int level) throw() {
    // 第一个源操作数与目的操作数相等，且第二个源操作数是立即数时，直接对内存做加法运算
    if (c_iter->method1_ == c_iter->method3_ && c_iter->src1_ == c_iter->dst_ &&
        Quaternary::IMMEDIATE_ADDRESSING == c_iter->method2_) {
        OpGeneralImmediate(ADD, c_iter->method3_, c_iter->dst_, Quaternary::NIL_ADDRESSING, 0,
                           para_num, var_space, level, c_iter->src2_);
    }
    // 第二个源操作数与目的操作数相等，且第一个源操作数是立即数时，直接对内存做加法运算
    else if (c_iter->method2_ == c_iter->method3_ && c_iter->src2_ == c_iter->dst_ &&
             Quaternary::IMMEDIATE_ADDRESSING == c_iter->method1_) {
        OpGeneralImmediate(ADD, c_iter->method3_, c_iter->dst_, Quaternary::NIL_ADDRESSING, 0,
                           para_num, var_space, level, c_iter->src1_);
    }
    // 其余情况，当第一个操作数是立即数时
    else {
        if (Quaternary::IMMEDIATE_ADDRESSING == c_iter->method1_) {
            // 加载第二个操作数到EAX
            OpRegisterGeneral(MOV, EAX, c_iter->method2_, c_iter->src2_, Quaternary::NIL_ADDRESSING,
                              0, para_num, var_space, level);
            // 加上第一个立即数
            OpRegisterGeneral(ADD, EAX, c_iter->method1_, c_iter->src1_, Quaternary::NIL_ADDRESSING,
                              0, para_num, var_space, level);
        } else // 当第一个操作数不是立即数时
        {
            // 加载第一个操作数到EAX
            OpRegisterGeneral(MOV, EAX, c_iter->method1_, c_iter->src1_, Quaternary::NIL_ADDRESSING,
                              0, para_num, var_space, level);
            // 加上第二个操作数
            OpRegisterGeneral(ADD, EAX, c_iter->method2_, c_iter->src2_, Quaternary::NIL_ADDRESSING,
                              0, para_num, var_space, level);
        }
        // 将EAX中的结果保存到目的操作数
        OpGeneralRegister(MOV, c_iter->method3_, c_iter->dst_, Quaternary::NIL_ADDRESSING, 0,
...
void AssemblyMaker::TranslateSub(const vector<Quaternary>::const_iterator& c_iter,
                                 int para_num,
                                 int var_space,
                                 int level) throw() {
    ...
    if (c_iter->method1_ == c_iter->method3_ && c_iter->src1_ == c_iter->dst_ &&
        Quaternary::IMMEDIATE_ADDRESSING == c_iter->method2_) {
        OpGeneralImmediate(SUB, c_iter->method3_, c_iter->dst_, Quaternary::NIL_ADDRESSING, 0,
                           para_num, var_space, level, c_iter->src2_);
    } else {
        // 装载被减数到EAX
        OpRegisterGeneral(MOV, EAX, c_iter->method1_, c_iter->src1_, Quaternary::NIL_ADDRESSING, 0,
                          para_num, var_space, level);
        // 减去第二个数
        OpRegisterGeneral(SUB, EAX, c_iter->method2_, c_iter->src2_, Quaternary::NIL_ADDRESSING, 0,
                          para_num, var_space, level);
        // 存储结果
        OpGeneralRegister(MOV, c_iter->method3_, c_iter->dst_, Quaternary::NIL_ADDRESSING, 0,
                          para_num, var_space, level);
...
// 目的操作数只可能是普通变量或临时变量
void AssemblyMaker::TranslateMul(const vector<Quaternary>::const_iterator& c_iter,
                                 int para_num,
                                 int var_space,
                                 int level) throw() {
    if (Quaternary::IMMEDIATE_ADDRESSING == c_iter->method1_) // 第一个操作数是立即数的情况
    {
        ...
        OpRegisterGeneral(MOV, EAX, c_iter->method1_, c_iter->src1_, Quaternary::NIL_ADDRESSING, 0,
                          para_num, var_space, level);
        // 乘上第二个操作数
        OpGeneral(IMUL, c_iter->method2_, c_iter->src2_, Quaternary::NIL_ADDRESSING, 0, para_num,
                  var_space, level);
    } else {
        // 加载第二个操作数到EAX
        OpRegisterGeneral(MOV, EAX, c_iter->method2_, c_iter->src2_, Quaternary::NIL_ADDRESSING, 0,
                          para_num, var_space, level);
        // 乘上第一个操作数
        OpGeneral(IMUL, c_iter->method1_, c_iter->src1_, Quaternary::NIL_ADDRESSING, 0, para_num,
                  var_space, level);
    }

    // 将EAX中的结果保存到目的操作数
    OpGeneralRegister(MOV, c_iter->method3_, c_iter->dst_, Quaternary::NIL_ADDRESSING, 0, para_num,
                      var_space, level);
}
...
void AssemblyMaker::TranslateDiv(const vector<Quaternary>::const_iterator& c_iter,
                                 int para_num,
                                 int var_space,
                                 int level) throw() {
    // 加载第一个操作数到EAX
    ...
    if (Quaternary::IMMEDIATE_ADDRESSING == c_iter->method2_) {
        OpRegisterGeneral(MOV, EBX, c_iter->method2_, c_iter->src2_, Quaternary::NIL_ADDRESSING, 0,
                          para_num, var_space, level);
    ...
    {
        assemble_buffer << "\n    CDQ"; // EDX相对EAX的符号位扩展
        OpGeneral(IDIV, c_iter->method2_, c_iter->src2_, Quaternary::NIL_ADDRESSING, 0, para_num,
                  var_space, level);
    }
    // 将EAX中的结果保存到目的操作数
    OpGeneralRegister(MOV, c_iter->method3_, c_iter->dst_, Quaternary::NIL_ADDRESSING, 0, para_num,
...
// 目的操作数可以是普通变量和临时变量
void AssemblyMaker::TranslateAssign(const vector<Quaternary>::const_iterator& c_iter,
                                    int para_num,
                                    int var_space,
                                    int level) throw() {
    // 源操作数是立即数时，可直接存储至内存
    if (Quaternary::IMMEDIATE_ADDRESSING == c_iter->method1_) {
        OpGeneralImmediate(MOV, c_iter->method3_, c_iter->dst_, Quaternary::NIL_ADDRESSING, 0,
                           para_num, var_space, level, c_iter->src1_);
    } else {
        // 将源操作数装载到EAX中
        OpRegisterGeneral(MOV, EAX, c_iter->method1_, c_iter->src1_, c_iter->method2_,
        ...
        OpGeneralRegister(MOV, c_iter->method3_, c_iter->dst_, Quaternary::NIL_ADDRESSING, 0,
                          para_num, var_space,
                          level); // stupid bug fixed by mxf at 20:17 2/2 2016 (0 for parameter level)
    }
}
...
// 目的操作数为数组
void AssemblyMaker::TranslateArrayAssign(const vector<Quaternary>::const_iterator& c_iter,
                                         int para_num,
                                         int var_space,
                                         int level) throw() {
    // 源操作数是立即数时，可直接存储至内存
    if (Quaternary::IMMEDIATE_ADDRESSING == c_iter->method1_) {
        OpArrayImmediate(MOV, c_iter->dst_, c_iter->method2_, c_iter->offset2_, para_num, level,
                         c_iter->src1_);
    } else {
        // 加载变量到EAX
...
// 无条件跳转
void AssemblyMaker::TranslateJmp(const vector<Quaternary>::const_iterator& c_iter,
                                 int para_num,
                                 int var_space,
                                 int level) throw() {
...
// 左右操作数相等时跳转
void AssemblyMaker::TranslateJe(const vector<Quaternary>::const_iterator& c_iter,
                                int para_num,
                                int var_space,
                                int level) throw() {
    OpRegisterGeneral(MOV, EAX, c_iter->method1_, c_iter->src1_, Quaternary::NIL_ADDRESSING, 0,
                      para_num, var_space, level);
    OpRegisterGeneral(CMP, EAX, c_iter->method2_, c_iter->src2_, Quaternary::NIL_ADDRESSING, 0,
                      para_num, var_space, level);
    assemble_buffer << "\n    je     " << GenerateLabelString(c_iter->dst_);
}
// 左右操作数不等时跳转
void AssemblyMaker::TranslateJne(const vector<Quaternary>::const_iterator& c_iter,
                                 int para_num,
                                 int var_space,
                                 int level) throw() {
    OpRegisterGeneral(MOV, EAX, c_iter->method1_, c_iter->src1_, Quaternary::NIL_ADDRESSING, 0,
...
// 左>右则跳转
void AssemblyMaker::TranslateJg(const vector<Quaternary>::const_iterator& c_iter,
                                int para_num,
                                int var_space,
                                int level) throw() {
    OpRegisterGeneral(MOV, EAX, c_iter->method1_, c_iter->src1_, Quaternary::NIL_ADDRESSING, 0,
...
// 左<=右则跳转
void AssemblyMaker::TranslateJng(const vector<Quaternary>::const_iterator& c_iter,
                                 int para_num,
                                 int var_space,
                                 int level) throw() {
    OpRegisterGeneral(MOV, EAX, c_iter->method1_, c_iter->src1_, Quaternary::NIL_ADDRESSING, 0,
...
// 左<右则跳转
void AssemblyMaker::TranslateJl(const vector<Quaternary>::const_iterator& c_iter,
                                int para_num,
                                int var_space,
                                int level) throw() {
    OpRegisterGeneral(MOV, EAX, c_iter->method1_, c_iter->src1_, Quaternary::NIL_ADDRESSING, 0,
...
// 左>=右则跳转
void AssemblyMaker::TranslateJnl(const vector<Quaternary>::const_iterator& c_iter,
                                 int para_num,
                                 int var_space,
                                 int level) throw() {
    OpRegisterGeneral(MOV, EAX, c_iter->method1_, c_iter->src1_, Quaternary::NIL_ADDRESSING, 0,
...
void AssemblyMaker::TranslateLabel(const vector<Quaternary>::const_iterator& c_iter,
                                   int para_num,
                                   int var_space,
                                   int level) throw() {
...
void AssemblyMaker::TranslateSetP(const vector<Quaternary>::const_iterator& c_iter,
                                  int para_num,
                                  int var_space,
                                  int level) throw() {
    OpGeneral(PUSH, c_iter->method3_, c_iter->dst_, c_iter->method2_, c_iter->offset2_, para_num,
              var_space, level);
}

// 将参数的地址压栈
// 参数可能是立即数、普通变量、数组、临时变量
void AssemblyMaker::TranslateSetRefP(const vector<Quaternary>::const_iterator& c_iter,
                                     int para_num,
                                     int var_space,
                                     int level) throw() {
    // 先将参数地址加载到EAX
    OpRegisterGeneral(LEA, EAX, c_iter->method3_, c_iter->dst_, c_iter->method2_, c_iter->offset2_,
                      para_num, var_space, level);
    // 再将EAX压栈
    assemble_buffer << "\n    push    eax";
}
...
void AssemblyMaker::TranslateCall(const vector<Quaternary>::const_iterator& c_iter,
                                  int para_num,
                                  int var_space,
                                  int level) throw() {
    // 一. 获得被调用函数的level
...
// 将目的操作数装入EAX，并进行函数返回
void AssemblyMaker::TranslateRet(const vector<Quaternary>::const_iterator& c_iter,
                                 int para_num,
                                 int var_space,
                                 int level) throw() {
    OpRegisterGeneral(MOV, EAX, c_iter->method3_, c_iter->dst_, c_iter->method2_, c_iter->offset2_,
                      para_num, var_space, level);
...
void AssemblyMaker::TranslateStore(const vector<Quaternary>::const_iterator& c_iter,
                                   int para_num,
                                   int var_space,
                                   int level) throw() {
    OpGeneralRegister(MOV, c_iter->method3_, c_iter->dst_, Quaternary::NIL_ADDRESSING, 0, para_num,
...
void AssemblyMaker::TranslateRead(const vector<Quaternary>::const_iterator& c_iter,
                                  int para_num,
                                  int var_space,
                                  int level) throw() {
    // 将要读取的变量的地址装入EAX
...
// 输出dst[offset]
void AssemblyMaker::TranslateWrite(const vector<Quaternary>::const_iterator& c_iter,
                                   int para_num,
                                   int var_space,
                                   int level) throw() {
    // 将操作数压栈
...
// 单操作数的运算
void AssemblyMaker::OpGeneral(enum SINGLEOPERATOR op,
                              Quaternary::AddressingMethod addressingmethod,
                              int index_or_value,
                              Quaternary::AddressingMethod array_addr_method,
                              int array_offset,
                              int para_num,
                              int var_space,
                              int level) throw() {
    if (Quaternary::IMMEDIATE_ADDRESSING == addressingmethod) {
        ...
        assemble_buffer << "\n    " << SingleOperatorName[op] << "    offset  _String"
                        << index_or_value;
    } else {
        assert(false);
    }
...
void AssemblyMaker::OpVar(enum SINGLEOPERATOR op,
                          int tokentable_index,
                          int para_num,
                          int level) throw() {
    // 确定变量的层次
    ...
    if (level == func_level)                                 // 判断是否为局部变量
    {
        ...
        if (n < para_num) // 说明是参数
        {
            ...
            assemble_buffer << "\n    " << SingleOperatorName[op] << "    dword ptr SS:[ebp + "
                            << offset << "]";
        } else // 说明是局部变量
        {
...
// para#n.addr = EBP + 4 * (1 + level + para_num - n)
// display#n.addr = EBP + 4 * (1 + level - n)
void AssemblyMaker::OpReference(enum SINGLEOPERATOR op,
                                int tokentable_index,
                                int para_num,
                                int level) throw() {
    // 确定变量的层次
...
void AssemblyMaker::OpArray(enum SINGLEOPERATOR op,
                            int tokentable_index,
                            Quaternary::AddressingMethod array_addr_method,
                            int array_offset,
                            int para_num,
    ...
    if (level == func_level) // 判断是否为局部变量
    {
        ...
        {
            // 1. 先把数组下标的变量的值放入ECX
            OpRegisterGeneral(MOV, ECX, array_addr_method, array_offset, Quaternary::NIL_ADDRESSING,
                              0, 0, 0, level);
...
void AssemblyMaker::OpGeneralRegister(enum DOUBLEOPERATOR op,
                                      Quaternary::AddressingMethod addressingmethod,
                                      int index,
                                      Quaternary::AddressingMethod array_addr_method,
                                      int array_offset,
...
// 根据取址方式的不同，调用不同的存储函数
// 第二个参数可能表示符号表中的index，也可能表示临时变量的index，故只命名为index
void AssemblyMaker::OpGeneralImmediate(enum DOUBLEOPERATOR op,
                                       Quaternary::AddressingMethod addressingmethod,
                                       int index,
                                       Quaternary::AddressingMethod array_addr_method,
                                       int array_offset,
...
// var#n.addr = EBP - 4 * (n -para_num + 1)
// display#n.addr = EBP + 4 * (1 + level - n)
void AssemblyMaker::OpVarRegister(enum DOUBLEOPERATOR op,
                                  int tokentable_index,
                                  int para_num,
                                  int level,
                                  enum REGISTER reg) throw() {
    ...
    if (level == func_level)                                 // 判断变量是否为函数的局部变量
    {
        ...
        {
            offset = 4 * (1 + level + para_num - n);
            assemble_buffer << "\n    " << DoubleOperatorName[op] << "    SS:[ebp + " << offset
                            << "], " << RegisterName[reg];
        } else // 说明是局部变量
        {
            offset = 4 * (n - para_num + 1);
    ...
    } else // 外层次的变量
    {
        ...
        if (n < extern_para_num) // 说明是外层参数
        {
            ...
            assemble_buffer << "\n    " << DoubleOperatorName[op] << "    SS:[ebx + " << offset
                            << "], " << RegisterName[reg];
        } else // 说明是外层局部变量
        {
...
// var#n.addr = EBP - 4 * (n -para_num + 1)
// display#n.addr = EBP + 4 * (1 + level - n)
void AssemblyMaker::OpVarImmediate(enum DOUBLEOPERATOR op,
                                   int tokentable_index,
                                   int para_num,
                                   int level,
                                   int immediate_value) throw() {
    ...
    if (level == func_level)                                 // 判断变量是否为函数的局部变量
    {
        ...
        {
            offset = 4 * (1 + level + para_num - n);
            assemble_buffer << "\n    " << DoubleOperatorName[op] << "    dword ptr SS:[ebp + "
    ...
    } else // 外层次的变量
    {
        ...
        {
            int offset = 4 * (1 + func_level + extern_para_num - n);
            assemble_buffer << "\n    " << DoubleOperatorName[op] << "    dword ptr SS:[ebx + "
                            << offset << "], " << immediate_value;
        } else // 说明是外层局部变量
        {
            int offset = 4 * (n - extern_para_num + 1);
...
// var#n.addr = EBP - 4 * (n -para_num + 1)
// display#n.addr = EBP + 4 * (1 + level - n)
void AssemblyMaker::OpReferenceRegister(enum DOUBLEOPERATOR op,
                                        int tokentable_index,
                                        int para_num,
                                        int level,
                                        enum REGISTER reg) throw() {
...
// var#n.addr = EBP - 4 * (n -para_num + 1)
// display#n.addr = EBP + 4 * (1 + level - n)
void AssemblyMaker::OpReferenceImmediate(enum DOUBLEOPERATOR op,
                                         int tokentable_index,
                                         int para_num,
                                         int level,
                                         int immediate_value) throw() {
...
// array#n.addr = EBP - 4 * (n - para_num + 1 + array_offset)
// display#n.addr = EBP + 4 * (1 + level - n)
void AssemblyMaker::OpArrayRegister(enum DOUBLEOPERATOR op,
                                    int tokentable_index,
                                    Quaternary::AddressingMethod array_addr_method,
                                    int array_offset,
                                    int para_num,
...
// array#n.addr = EBP - 4 * (n - para_num + 1 + array_offset)
// display#n.addr = EBP + 4 * (1 + level - n)
void AssemblyMaker::OpArrayImmediate(enum DOUBLEOPERATOR op,
                                     int tokentable_index,
                                     Quaternary::AddressingMethod array_addr_method,
                                     int array_offset,
                                     int para_num,
    ...
    if (level == func_level) // 判断是否为局部变量
    {
        ...
        } else // 否则，就要取得数组下标（某个变量）的值，在汇编程序中计算其偏移量
        {
            ...
            assemble_buffer << "\n    sub     ecx,    ebp"
                            << "\n    neg     ecx";
            // 3. 执行该执行的操作
            assemble_buffer << "\n    " << DoubleOperatorName[op]
                            << "    dword ptr dword ptr SS:[ecx], " << immediate_value;
        }
    } else // 外层次的变量
    {
        // 一. 获得外层次的EBP的值，加载到EBX中
        ...
        } else // 否则，就要取得数组下标（某个变量）的值，在汇编程序中计算其偏移量
        {
            ...
            assemble_buffer << "\n    sub     ecx, ebx"
                            << "\n    neg     ecx";
            // 3. 执行该执行的操作
            assemble_buffer << "\n    " << DoubleOperatorName[op] << "    dword ptr SS:[ecx], "
                            << immediate_value;
        }
    }
}
...
// 地址计算公式：
// temp#n.addr = EBP - 4 * (var_space + n + 1)
void AssemblyMaker::OpTempRegister(enum DOUBLEOPERATOR op,
                                   int index,
                                   int var_space,
                                   enum REGISTER reg) throw() {
    // 这里的index就是上式中的n
...
// 地址计算公式：
// temp#n.addr = EBP - 4 * (var_space + n + 1)
void AssemblyMaker::OpTempImmediate(enum DOUBLEOPERATOR op,
                                    int index,
                                    int var_space,
                                    int immediate_value) throw() {
    // 这里的index就是上式中的n
    int offset = 4 * (var_space + index + 1);
    assemble_buffer << "\n    " << DoubleOperatorName[op] << "     dword ptr SS:[ebp - " << offset
                    << "], " << immediate_value;
}

// 双操作数的运算（第一个操作数是寄存器，第二个操作数是各种类型的内存变量或立即数）
void AssemblyMaker::OpRegisterGeneral(enum DOUBLEOPERATOR op,
                                      enum REGISTER reg,
                                      Quaternary::AddressingMethod addressingmethod,
                                      int index_or_value,
                                      Quaternary::AddressingMethod array_addr_method,
    ...
    if (Quaternary::IMMEDIATE_ADDRESSING == addressingmethod) {
        ...
        assemble_buffer << "\n    " << DoubleOperatorName[op] << "    offset  _String"
                        << index_or_value;
    } else {
        assert(false);
    }
...
void AssemblyMaker::OpRegisterImmediate(enum DOUBLEOPERATOR op, enum REGISTER reg, int value) throw() {
    //TODO 通过输出格式控制来优化效率
    assemble_buffer << "\n    " << DoubleOperatorName[op] << "    " << RegisterName[reg] << ", "
                    << value;
}
...
// var#n.addr = EBP - 4 * (n -para_num + 1)
// display#n.addr = EBP + 4 * (1 + level - n)
void AssemblyMaker::OpRegisterVar(enum DOUBLEOPERATOR op,
                                  enum REGISTER reg,
                                  int tokentable_index,
                                  int para_num,
                                  int level) throw() {
...
// para#n.addr = EBP + 4 * (1 + level + para_num - n)
// display#n.addr = EBP + 4 * (1 + level - n)
void AssemblyMaker::OpRegisterReference(enum DOUBLEOPERATOR op,
                                        enum REGISTER reg,
                                        int tokentable_index,
                                        int para_num,
                                        int level) throw() {
...
// array#n.addr = EBP - 4 * (n - para_num + 1 + array_offset)
// display#n.addr = EBP + 4 * (1 + level - n)	这里的n即为外层函数的层次
void AssemblyMaker::OpRegisterArray(enum DOUBLEOPERATOR op,
                                    enum REGISTER reg,
                                    int tokentable_index,
                                    Quaternary::AddressingMethod array_addr_method,
                                    int array_offset,
...
// 地址计算公式：
// temp#n.addr = EBP - 4 * (var_space + n + 1)
void AssemblyMaker::OpRegisterTemp(enum DOUBLEOPERATOR op,
                                   enum REGISTER reg,
                                   int index,
                                   int var_space) throw() {
    // 这里的index就是上式中的n
