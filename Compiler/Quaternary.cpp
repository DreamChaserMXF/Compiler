...
const char* Quaternary::OPCodeString[] = {"NIL_OP", "NEG",       "ADD",       "SUB",   "MUL",
                                          "DIV",    "ASG",       "AASG",      "STORE", "JMP",
                                          "JE",     "JNE",       "JG",        "JNG",   "JL",
                                          "JNL",    "FUNC_CALL", "PROC_CALL", "READ",  "WRITE",
                                          "SETP",   "SETREFP",   "RET",       "BEGIN", "END",
                                          "LABEL"};

Quaternary::Quaternary() throw()
    : op_(NIL_OP)
    , method1_(NIL_ADDRESSING)
    , src1_(0)
    , method2_(NIL_ADDRESSING)
    , src2_(0)
    , method3_(NIL_ADDRESSING)
    , dst_(0) {}
Quaternary::Quaternary(OPCode op,
                       AddressingMethod method1,
                       int src1,
                       AddressingMethod method2,
                       int src2,
                       AddressingMethod method3,
                       int dst) throw()
    : op_(op)
    , method1_(method1)
    , src1_(src1)
    ...
    , method3_(method3)
    , dst_(dst)
    , dst_decoratetype_(TokenTableItem::VOID) {}
...
std::vector<Quaternary>::iterator Quaternary::FindTempVar(
    const std::vector<Quaternary>::iterator& begin_iter) throw() {
...
std::vector<Quaternary>::const_iterator Quaternary::GetFunctionBody(
    std::vector<Quaternary>::const_iterator begin_iter) throw() {
    ++begin_iter;
