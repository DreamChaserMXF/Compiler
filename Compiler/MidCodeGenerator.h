...
private:
    ...
    void PrintFunctionFrame(const char* func_name,
                            size_t depth) throw(); // 输出函数帧信息，depth是语法分析时的函数调用深度
    ...
    void AssigningStatement(const Token& idToken,
                            TokenTable::iterator& iter,
                            size_t depth) throw(); // 赋值语句
    ...
    Quaternary::OPCode ConvertFromToken(const Token::TokenType& token_type, bool inverse) const
        throw();
    ...
    void ProcFuncCallStatement(const Token token_,
                               const vector<ExpressionAttribute>& parameter_attributes,
                               size_t depth) throw(); // 函数调用语句
    void ArgumentList(const vector<ExpressionAttribute>& parameter_attributes,
