...
TokenTable::TokenTable() throw()
    : rows_(), subroutine_tokentableindex_stack_(), addr_(0), subroutine_tokentableaddr_stack_() {
    subroutine_tokentableindex_stack_.push(
        0); // 这行有必要，因为在SearchDefinitionInCurrentLevel的时候要用到top元素
    subroutine_tokentableaddr_stack_.push(0); // 这行似乎没有必要
}

TokenTableItem TokenTable::at(int index) const throw(std::out_of_range) { return rows_.at(index); }
...
bool TokenTable::SearchDefinitionInCurrentLevel(const string& name) throw() {
    ...
    if (iter != rows_.end()) {
        ...
        while (iter != rows_.end() &&
               (false == iter->valid_ || iter->level_ != current_level || iter->name_ != name)) {
            ++iter;
        }
    }
...
TokenTable::iterator TokenTable::SearchDefinition(const Token& token) throw() {
    ...
    if (iter != rows_.end()) {
        int current_level = iter->level_;
        while (iter != rows_.end() &&
               (false == iter->valid_ || iter->level_ != current_level ||
    ...
    } else // 在之前层寻找定义
    {
        ...
        if (r_iter != rows_.rend()) // 如果之前层不是顶层
        {
            ...
            while (r_iter != rows_.rend() &&
                   (false == r_iter->valid_ || r_iter->level_ > last_level ||
                ...
                if (true == r_iter->valid_ &&
                    r_iter->level_ <
                        last_level) // bug fixed[if(true == iter->valid_ && r_iter->level_ < last_level)]
                {
...
TokenTable::const_iterator TokenTable::SearchDefinition(const Token& token) const throw() {
    ...
    {
        const_reverse_iterator r_iter(rows_.begin() +
                                      subroutine_tokentableindex_stack_.top()); //逆向迭代器
        if (r_iter != rows_.rend()) // 如果之前层不是顶层
        {
            int last_level = r_iter->level_; // 上一层次为当前层入口的过程或函数的层次
            while (r_iter != rows_.rend() &&
...
TokenTable::const_iterator TokenTable::begin() const throw() { return rows_.begin(); }
TokenTable::const_iterator TokenTable::end() const throw() { return rows_.end(); }
//const TokenTableItem& TokenTable::back() const throw()
//{
//	return rows_.back();
//}
size_t TokenTable::size() const throw() { return rows_.size(); }
...
void TokenTable::Print(std::ostream& output) const throw() { output << toString(); }

void TokenTable::AddConstItem(Token constIdentifier,
                              TokenTableItem::DecorateType decoratetype_,
                              int value,
                              int level) throw() {
    TokenTableItem item(constIdentifier.value_.identifier, TokenTableItem::CONST, decoratetype_,
                        value, level, constIdentifier.lineNumber_, addr_++);
    rows_.push_back(item);
}
void TokenTable::AddVariableItem(Token variableIdentifier,
                                 TokenTableItem::DecorateType decoratetype_,
                                 int level) throw() {
    TokenTableItem item(variableIdentifier.value_.identifier, TokenTableItem::VARIABLE,
                        decoratetype_, 0, level, variableIdentifier.lineNumber_, addr_++);
    rows_.push_back(item);
}
void TokenTable::AddArrayItem(Token arrayIdentifier,
                              TokenTableItem::DecorateType decoratetype_,
                              int arrayLength,
                              int level) throw() {
    TokenTableItem item(arrayIdentifier.value_.identifier, TokenTableItem::ARRAY, decoratetype_,
                        arrayLength, level, arrayIdentifier.lineNumber_, addr_);
    rows_.push_back(item);
...
int TokenTable::AddProcedureItem(Token procedureIdentifier, int level) throw() {
    ...
    TokenTableItem item(procedureIdentifier.value_.identifier, TokenTableItem::PROCEDURE,
                        TokenTableItem::VOID, 0, level, procedureIdentifier.lineNumber_, addr_);
    rows_.push_back(item);
    return rows_.size() - 1;
}
int TokenTable::AddFunctionItem(Token functionIdentifier, int level) throw() {
    TokenTableItem item(functionIdentifier.value_.identifier, TokenTableItem::FUNCTION,
                        TokenTableItem::VOID, 0, level, functionIdentifier.lineNumber_, addr_++);
    rows_.push_back(item);
    return rows_.size() - 1;
}
// 设置过程/函数的参数个数
...
void TokenTable::SetFunctionReturnType(const string& func_name,
                                       TokenTableItem::DecorateType decoratetype_) throw() {
    reverse_iterator iter = rows_.rbegin();
...
void TokenTable::AddParameterItem(Token parameterIdentifier,
                                  TokenTableItem::DecorateType decoratetype_,
                                  int level) throw() {
    TokenTableItem item(parameterIdentifier.value_.identifier, TokenTableItem::PARAMETER,
                        decoratetype_, 0, level, parameterIdentifier.lineNumber_, addr_++);
    rows_.push_back(item);
}
...
int TokenTable::GetVariableSpace(TokenTable::const_iterator c_iter) const throw() {
    ...
    // 如果该过程/函数没有局部变量
    if (rows_.end() == c_iter || TokenTableItem::PROCEDURE == c_iter->itemtype_ ||
    ...
    while (rows_.end() != c_iter && TokenTableItem::PROCEDURE != c_iter->itemtype_ &&
           TokenTableItem::FUNCTION != c_iter->itemtype_) {
        ++c_iter;
    }
    ...
    return item.addr_ - first_var_addr +
        ((TokenTableItem::ARRAY == item.itemtype_) ? item.value_ : 1);
}