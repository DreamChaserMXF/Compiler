...
private:
    ...
    void ParseCurrentToken(Token& token, char& ch) throw(
        LexException); // 根据已读到的当前字符，分析完整个token，并读入下一个字符
    ...
    void PlusHandle(Token& token, char& ch) throw(
        LexException); // 处理加号(单操作符或双操作符)，注意，这里的token要存储上一个识别出的单词
    void MinusHandle(Token& token, char& ch) throw(
        LexException); // 处理减号(单操作符或双操作符)，注意，这里的token要存储上一个识别出的单词
