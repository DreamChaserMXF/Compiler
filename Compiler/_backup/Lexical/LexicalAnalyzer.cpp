...
LexicalAnalyzer::LexicalAnalyzer(const string& srcFileName) throw()
    : srcfile_(srcFileName), currentline_(0), token_vector_(), token_iter_(), string_set() {}

bool LexicalAnalyzer::IsBound() const throw() { return srcfile_.is_open(); }
...
void LexicalAnalyzer::Print(ostream& output) const throw() {
    int tokenNo = 1;
    output << "TokenNumber" << '\t' << "LineNumber" << '\t' << "TokenType"
           << "\t    "
           << "TokenValue" << std::endl;
    for (vector<Token>::const_iterator iter = token_vector_.begin(); iter != token_vector_.end();
         ++iter) {
...
void LexicalAnalyzer::LetterHandle(Token& token, char& ch) throw() // 处理字母
{
    ...
    // 在保留字表中查找，是否为保留字
    map<string, Token::TokenType>::const_iterator iter =
        Token::sReserveWordToTokenType.find(token.value_.identifier);
...
void LexicalAnalyzer::StringHandle(Token& token, char& ch) throw(LexException) // 处理字符串常量
{
    ...
    if ('"' == ch) // 判断结束符号是不是双引号
    {
        ...
        throw LexException("wrong string constant definition: require \" as an end sign", ch,
                           currentline_);
    }
}
void LexicalAnalyzer::CharHandle(Token& token, char& ch) throw(LexException) // 处理字符常量
{
    ...
    {
        throw LexException("wrong character constant definition: require letter after \"'\"", ch,
                           currentline_);
    } else // 读取结束的单引号
    {
        ...
        {
            throw LexException("wrong character constant definition: require \"'\"", ch,
                               currentline_);
        }
    }
}
