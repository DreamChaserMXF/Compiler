...
private:
    ...
    void PrintFunctionFrame(const char* func_name,
                            int depth) throw(); // 输出函数帧信息，depth是语法分析时的函数调用深度
    ...
    vector<int> CaseElement(const ExpressionAttribute& exp_attribute,
                            int caselabel,
                            int endlabel,
    ...
    void ProcedureCallStatement(const Token token_,
                                const vector<TokenTableItem::DecorateType>& parameter_decorate_types,
                                int depth) throw(); // 过程调用语句
    void FunctionCallStatement(const Token token_,
                               const vector<TokenTableItem::DecorateType>& parameter_decorate_types,
                               int depth) throw(); // 函数调用语句
