...
SyntaxAnalyzer::SyntaxAnalyzer(LexicalAnalyzer& lexical_analyzer,
                               ...
                               vector<Quaternary>& quaternarytable) throw()
...
string SyntaxAnalyzer::toString() const throw() { return syntax_info_buffer_.str(); }
...
void SyntaxAnalyzer::Routine(int depth) throw() {
    ...
    // 插入主函数的BEGIN
    Quaternary q_mainbegin(Quaternary::BEGIN, Quaternary::NIL_OPERAND, 0,
                           Quaternary::IMMEDIATE_OPERAND, 0, Quaternary::IMMEDIATE_OPERAND, -1);
    quaternarytable_.push_back(q_mainbegin);
    // 解析分程序
    SubRoutine(depth + 1);
    // 判断结束符号
    ...
    // 插入主函数的END
    Quaternary q_mainend(Quaternary::END, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND, 0,
                         Quaternary::IMMEDIATE_OPERAND, -1);
...
void SyntaxAnalyzer::SubRoutine(int depth) throw() {
    ...
    if (token_.type_ == Token::BEGIN) {
        ...
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  didn't find sentence block in subroutine\n";
        return;
    }
}
...
void SyntaxAnalyzer::ConstantPart(int depth) throw() {
    ...
    if (token_.type_ != Token::SEMICOLON) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be ';' after constant definition\n";
        /*while(lexical_analyzer_.GetNextToken(token_) && token_.type_ != Token::SEMICOLON)
		{ }*/
        is_successful_ = false;
        if (Token::VAR == token_.type_ || Token::PROCEDURE == token_.type_ ||
            Token::FUNCTION == token_.type_ ||
            Token::BEGIN == token_.type_) //	这里可能是忘记加分号了，所以直接返回而不读取下一个单词
        {
            return;
        } else {
            while (token_.type_ != Token::NIL &&
                   token_.type_ !=
                       Token::SEMICOLON) // 若是其他单词，表示常量说明部分还未结束，故要读到下一个分号
            {
                lexical_analyzer_.GetNextToken(token_);
            };
            lexical_analyzer_.GetNextToken(token_);
...
void SyntaxAnalyzer::constantDefination(int depth) throw() {
    ...
    if (token_.type_ != Token::IDENTIFIER) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be identifier at the beginning of constant definition\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::COMMA &&
               token_.type_ != Token::SEMICOLON) // 读到下一个逗号或分号
        {
            lexical_analyzer_.GetNextToken(token_);
        }
        return;
    ...
    lexical_analyzer_.GetNextToken(token_);
    if (token_.type_ != Token::CONST_INTEGER && token_.type_ != Token::CONST_CHAR) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be constant integer or character after '='\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::COMMA &&
               token_.type_ != Token::SEMICOLON) // 读到下一个逗号或分号
        {
...
void SyntaxAnalyzer::VariablePart(int depth) throw() {
    ...
    do {
        ...
        if (token_.type_ != Token::SEMICOLON) {
            std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                      << "  should be ';' after variable definition\n";
            is_successful_ = false;
            if (Token::IDENTIFIER == token_.type_) // 忘记加分号，后面还有变量定义
            {
                ...
                while (Token::NIL != token_.type_ && Token::SEMICOLON != token_.type_ &&
                       Token::PROCEDURE != token_.type_ && Token::FUNCTION != token_.type_ &&
                       Token::BEGIN == token_.type_) {
                    lexical_analyzer_.GetNextToken(token_);
                }
...
void SyntaxAnalyzer::VariableDefinition(int depth) throw() {
    ...
    if (token_.type_ != Token::IDENTIFIER) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be identifier at the beginning of variable definition\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::IDENTIFIER && token_.type_ != Token::PROCEDURE &&
               token_.type_ != Token::FUNCTION &&
               token_.type_ != Token::BEGIN) // 读到结尾、分号、下一个标识符或PROCEDURE FUNCTION BEGIN
        {
            lexical_analyzer_.GetNextToken(token_);
        }
    ...
    if (token_.type_ != Token::COLON) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be ':' after identifier to specify the type\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::RW_INTEGER &&
               token_.type_ != Token::RW_CHAR && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::PROCEDURE && token_.type_ != Token::FUNCTION &&
               token_.type_ != Token::BEGIN) // 读到结尾、类型说明符、分号、PROCEDURE FUNCTION BEGIN
        {
            lexical_analyzer_.GetNextToken(token_);
        }
...
void SyntaxAnalyzer::TypeSpecification(int depth) throw() {
    ...
    if (token_.type_ == Token::ARRAY) {
        ...
        if (token_.type_ != Token::LEFT_BRACKET) {
            std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                      << "  should be '[' after \"array\"\n";
            is_successful_ = false;
            while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
                   token_.type_ != Token::PROCEDURE && token_.type_ != Token::FUNCTION &&
                   token_.type_ != Token::BEGIN) // 读到结尾、分号、PROCEDURE FUNCTION BEGIN
            {
                lexical_analyzer_.GetNextToken(token_);
            }
        ...
        if (token_.type_ != Token::CONST_INTEGER) {
            std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                      << "  should be a constant integer after '['\n";
            is_successful_ = false;
            while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
                   token_.type_ != Token::PROCEDURE && token_.type_ != Token::FUNCTION &&
                   token_.type_ != Token::BEGIN) // 读到结尾、分号、PROCEDURE FUNCTION BEGIN
    ...
    if (token_.type_ != Token::RW_INTEGER && token_.type_ != Token::RW_CHAR) // 若没有类型说明
    {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be \"integer\" or \"char\" for type specification\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::PROCEDURE && token_.type_ != Token::FUNCTION &&
               token_.type_ != Token::BEGIN) // 读到结尾、分号、PROCEDURE FUNCTION BEGIN
    ...
    if (TokenTableItem::ARRAY == itemtype_) // 若是数组
    {
        for (vector<Token>::const_iterator iter = tokenbuffer_.begin(); iter != tokenbuffer_.end();
             ++iter) {
            if (tokentable_.SearchDefinitionInCurrentLevel(
                    iter->value_.identifier)) // 重定义后，仍然插入当前定义（也可不插入）
            {
                std::cout << "line " << iter->lineNumber_ << ":  " << iter->toString()
                          << "  redifinition\n";
                is_successful_ = false;
            }
...
void SyntaxAnalyzer::ProcedurePart(int depth) throw() {
    ...
    do {
        ...
        SubRoutine(depth + 1);
        // 生成过程的END四元式
        quaternarytable_.push_back(Quaternary(Quaternary::END, Quaternary::NIL_OPERAND, 0,
                                              Quaternary::NIL_OPERAND, 0,
...
int SyntaxAnalyzer::ProcedureHead(int depth) throw() {
    ...
    {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be a procedure name after \"procedure\"\n";
        is_successful_ = false;
    ...
    if (tokentable_.SearchDefinitionInCurrentLevel(
            token_.value_.identifier)) // 重定义时，仍然插入重定义的过程定义（因为仍然插入后影响不大，而不插入的话，会影响该次的过程分程序的语法语义分析）
    {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  redifinition\n";
        is_successful_ = false;
    ...
    quaternarytable_.push_back(
        Quaternary(Quaternary::BEGIN, Quaternary::NIL_OPERAND, 0, Quaternary::IMMEDIATE_OPERAND,
    ...
    if (token_.type_ != Token::SEMICOLON) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  lost ';' at the end of procedure head\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::PROCEDURE && token_.type_ != Token::FUNCTION &&
               token_.type_ != Token::BEGIN) // 读到结尾、分号、PROCEDURE FUNCTION BEGIN
...
int SyntaxAnalyzer::FunctionHead(int depth) throw() {
    ...
    {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be a function name after \"function\"\n";
        is_successful_ = false;
        tokentable_.Locate(); // 这里进行定位，是为了抵消FunctionPart中的重定位
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::FUNCTION && token_.type_ != Token::BEGIN) // 读到结尾、分号、BEGIN
        {
            lexical_analyzer_.GetNextToken(token_);
        }
        if (Token::SEMICOLON == token_.type_) {
    ...
    if (tokentable_.SearchDefinitionInCurrentLevel(
            token_.value_.identifier)) // 重定义时，仍然插入重定义的过程定义（因为仍然插入后影响不大，而不插入的话，会影响该次的函数分程序的语法语义分析）
    {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  redifinition\n";
        is_successful_ = false;
    ...
    if (token_.type_ != Token::LEFT_PAREN) {
        ...
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::FUNCTION &&
               token_.type_ != Token::BEGIN) // 读到结尾、分号、FUNCTION BEGIN
        {
            lexical_analyzer_.GetNextToken(token_);
        }
        if (Token::SEMICOLON == token_.type_) // 分号后再读一个单词，可能会进入分程序
    ...
    {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  lost ':' after Parameter Statement\n";
        is_successful_ = false;
    }
    lexical_analyzer_.GetNextToken(token_);
    if (token_.type_ != Token::RW_INTEGER && token_.type_ != Token::RW_CHAR) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be \"integer\" or \"char\" after ':' to specify the return type\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::FUNCTION &&
               token_.type_ != Token::BEGIN) // 读到结尾、分号、FUNCTION BEGIN
    ...
    {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  lost ';' at the end of function head\n";
        is_successful_ = false;
        return func_index;
    }
    lexical_analyzer_.GetNextToken(token_);
...
int SyntaxAnalyzer::ParameterTerm(int depth) throw() {
    ...
    if (token_.type_ != Token::IDENTIFIER) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be parameter name surrounded by parentheses\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::COMMA &&
               token_.type_ != Token::SEMICOLON) // 读到结尾、分号、BEGIN
        {
            lexical_analyzer_.GetNextToken(token_);
        }
    ...
    while (Token::COMMA == token_.type_) {
        ...
        if (token_.type_ != Token::IDENTIFIER) {
            std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                      << "  should be parameter name after ','\n";
            is_successful_ = false;
            while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
                   token_.type_ != Token::IDENTIFIER) // 读到结尾、分号、BEGIN
            {
                lexical_analyzer_.GetNextToken(token_);
            }
            if (Token::IDENTIFIER != token_.type_) {
    ...
    if (token_.type_ != Token::COLON) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  lost ':' to specify the type after parameter name\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::PROCEDURE && token_.type_ != Token::FUNCTION &&
               token_.type_ != Token::BEGIN) // 读到结尾、分号、PROCEDURE FUNCTION BEGIN
    ...
    if (token_.type_ != Token::RW_INTEGER && token_.type_ != Token::RW_CHAR) {
        std::cout
            << "line " << token_.lineNumber_ << ":  " << token_.toString()
            << "  should be \"integer\" or \"char\" after ':' to specify the parameter type\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::PROCEDURE && token_.type_ != Token::FUNCTION &&
               token_.type_ != Token::BEGIN) // 读到结尾、分号、PROCEDURE FUNCTION BEGIN
...
vector<TokenTableItem::DecorateType> SyntaxAnalyzer::ArgumentList(int depth) throw() // 实参表
{
    ...
    // 生成设置参数的四元式
    Quaternary q_addpara(Quaternary::SETP, Quaternary::NIL_OPERAND, 0,
    ...
    if (Quaternary::TEMPORARY_OPERAND == argument_attribute.operandtype_ ||
        Quaternary::TEMPORARY_OPERAND ==
            argument_attribute.offset_operandtype_) // 两者不可能同时成立，故写在一起
    {
        --tempvar_index_;
    }
...
void SyntaxAnalyzer::StatementBlockPart(int depth) throw() // 复合语句
{
    ...
    if (token_.type_ != Token::END) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be \"end\" at the end of Statement Block\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::END) // 读到结尾或分号或END
        {
            lexical_analyzer_.GetNextToken(token_);
        }
        ...
        case Token::IDENTIFIER:
            ...
            if (Token::LEFT_PAREN == token_.type_) // 过程调用
            {
                ...
                vector<TokenTableItem::DecorateType> decorate_types =
                    tokentable_.GetProcFuncParameter(iter);
                // 生成过程调用四元式
                Quaternary q_procedurecall(Quaternary::PROC_CALL, Quaternary::NIL_OPERAND, 0,
                                           Quaternary::PARANUM_OPERAND, decorate_types.size(),
                                           Quaternary::PROC_FUNC_INDEX,
            ...
            {
                std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                          << "  please check the spelling of the assigning operator\n";
                is_successful_ = false;
                while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
                       token_.type_ != Token::END) // 读到结尾或分号或END
                {
...
void SyntaxAnalyzer::AssigningStatement(const Token& idToken,
                                        TokenTable::iterator& iter,
                                        int depth) // 赋值语句
{
    ...
    if (token_.type_ != Token::ASSIGN) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  \":=\" doesn't occur in the Assigning Statement\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::END) // 读到结尾或分号或END
        {
    ...
    if (iter->decoratetype_ < right_attribute.decoratetype_) // 小于表示不能从右至左的转换
    {
        std::cout << "line " << idToken.lineNumber_ << ":  " << idToken.toString()
                  << "  cannot convert from "
                  << TokenTableItem::DecorateTypeString[right_attribute.decoratetype_] << " to "
                  << TokenTableItem::DecorateTypeString[iter->decoratetype_] << "\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::END) // 读到结尾或分号或END
        {
...
ExpressionAttribute SyntaxAnalyzer::Factor(int depth) throw() // 因子
{
    ...
    if (Token::IDENTIFIER == token_.type_) {
        ...
        } else if (Token::LEFT_PAREN == token_.type_) // 左括号，函数调用
        {
            ...
            Quaternary q_functioncall(Quaternary::FUNC_CALL, Quaternary::NIL_OPERAND, 0,
                                      Quaternary::PARANUM_OPERAND,
                                      decorate_types.size(), // 函数的参数个数
                                      Quaternary::PROC_FUNC_INDEX,
                                      distance(tokentable_.begin(),
                                               static_cast<TokenTable::const_iterator>(iter)));
            ...
            Quaternary q_store(Quaternary::STORE, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND,
                               0, Quaternary::TEMPORARY_OPERAND, tempvar_index_++);
        ...
        } else // 单独一个标识符
        {
            ...
            if (iter->itemtype_ != TokenTableItem::VARIABLE &&
                ...
                std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                          << "  single token Factor should be varaible or constant\n";
                is_successful_ = false;
                while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
                       token_.type_ != Token::END) // 读到结尾或分号或END
                {
    ...
    } else if (Token::CONST_INTEGER == token_.type_) // 整型字面常量
    {
        ...
        factor_attribute.decoratetype_ =
            TokenTableItem::VOID; // 记录类型为VOID，因为它可以自然地向其他类型转换
        factor_attribute.operandtype_        = Quaternary::IMMEDIATE_OPERAND;
...
void SyntaxAnalyzer::IfStatement(int depth) throw() // 条件语句
{
    ...
    if (Token::THEN != token_.type_) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be \"then\" after Condition\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::END) // 读到结尾或分号或END
        {
    ...
    if (Token::ELSE == token_.type_) {
        ...
        // 生成无条件跳转语句
        Quaternary q_jmp(Quaternary::JMP, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND, 0,
                         Quaternary::IMMEDIATE_OPERAND, label2);
        quaternarytable_.push_back(q_jmp);
        // 设置第一个label
        Quaternary q_label1(Quaternary::LABEL, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND,
                            0, Quaternary::IMMEDIATE_OPERAND, label1);
        quaternarytable_.push_back(q_label1);
        // 读取else中的语句
        lexical_analyzer_.GetNextToken(token_);
        Statement(depth + 1);
        // 设置第二个label
        Quaternary q_label2(Quaternary::LABEL, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND,
                            0, Quaternary::IMMEDIATE_OPERAND, label2);
...
void SyntaxAnalyzer::Condition(int endlabel, int depth) throw() // 条件
{
    ...
    if (Token::LT != token_.type_ && Token::LEQ != token_.type_ && Token::GT != token_.type_ &&
        Token::GEQ != token_.type_ && Token::EQU != token_.type_ && Token::NEQ != token_.type_) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be logic operator in the middle of Condition\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::END) // 读到结尾或分号或END
        {
...
void SyntaxAnalyzer::CaseStatement(int depth) throw() // 情况语句
{
    ...
    if (Token::OF != token_.type_) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be \"of\" to specify the certain case\n";
        is_successful_ = false;
        // 这里假设是忘记写of，故不返回
    }
    // 为END处申请一个label
    ...
    // 执行完所有JE后，跳转到END处
    Quaternary q_endjmp(Quaternary::JMP, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND, 0,
                        Quaternary::IMMEDIATE_OPERAND, endlabel);
    quaternarytable_.insert(quaternarytable_.begin() + jmp_insertion_location, q_endjmp);

    // 插入结束的label
    Quaternary q_endlabel(Quaternary::LABEL, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND, 0,
                          Quaternary::IMMEDIATE_OPERAND, endlabel);
    ...
    if (Token::END != token_.type_) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be \"end\" at the end of case Statement\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::END) // 读到结尾或分号或END
        {
...
// <情况表元素> ::= <情况常量表>:<语句>
// <情况常量表> ::=  <常量 | 常变量>{, <常量 | 常变量>}
vector<int> SyntaxAnalyzer::CaseElement(const ExpressionAttribute& exp_attribute,
                                        int caselabel,
                                        int endlabel,
                                        int depth) throw() // 情况表元素
{
    ...
    if (Token::CONST_INTEGER != token_.type_ && Token::CONST_CHAR != token_.type_ &&
        tokentable_.end() == iter) {
        std::cout
            << "line " << token_.lineNumber_ << ":  " << token_.toString()
            << "  should be constant integer or character or constant variable after \"case\"\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::END) // 读到结尾或分号
        {
            lexical_analyzer_.GetNextToken(token_);
        }
    ...
    while (Token::COMMA == token_.type_) {
        ...
        if (Token::CONST_INTEGER != token_.type_ && Token::CONST_CHAR != token_.type_ &&
            tokentable_.end() == iter) {
            std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                      << "  should be constant integer or character after \"case\"\n";
            is_successful_ = false;
            while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
                   token_.type_ != Token::END) // 读到结尾或分号或END
            {
    ...
    if (Token::COLON != token_.type_) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be ':' after constant to specify the certain action\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::END) // 读到结尾或分号或END
        {
    ...
    Quaternary q_caselabel(Quaternary::LABEL, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND,
                           0, Quaternary::IMMEDIATE_OPERAND, caselabel);
...
void SyntaxAnalyzer::ReadStatement(int depth) throw() // 读语句
{
    ...
    if (Token::LEFT_PAREN != token_.type_) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be '(' to specify the arguments\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::END) // 读到结尾或分号或END
        {
    ...
    do {
        ...
        if (Token::IDENTIFIER != token_.type_) {
            std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                      << "  should be variable name in the location of read argument\n";
            is_successful_ = false;
            while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
                   token_.type_ != Token::END) // 读到结尾或分号或END
            {
        ...
        {
            std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                      << "  cannot be assigned in read call\n";
            is_successful_ = false;
            while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
                   token_.type_ != Token::END) // 读到结尾或分号或END
            {
        ...
        // 生成READ调用的四元式
        Quaternary q_read(Quaternary::READ, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND, 0,
                          Quaternary::VARIABLE_OPERAND,
                          distance(tokentable_.begin(),
                                   static_cast<TokenTable::const_iterator>(iter)));
...
void SyntaxAnalyzer::WriteStatement(int depth) throw() // 写语句
{
    ...
    if (Token::CONST_STRING == token_.type_) {
        ...
        // 生成WRITE调用的四元式
        Quaternary q_read(Quaternary::WRITE, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND, 0,
                          Quaternary::STRING_OPERAND,
        ...
        if (Token::COMMA == token_.type_) {
            ...
            ExpressionAttribute exp_attribute = Expression(depth + 1);
            // 生成WRITE调用的四元式
            Quaternary q_read(Quaternary::WRITE, Quaternary::NIL_OPERAND, 0,
            ...
            if (Quaternary::TEMPORARY_OPERAND == exp_attribute.operandtype_ ||
                Quaternary::TEMPORARY_OPERAND ==
                    exp_attribute.offset_operandtype_) // 两者不可能同时成立，故写在一起
            {
                --tempvar_index_;
            }
        }
...
void SyntaxAnalyzer::WhileLoopStatement(int depth) throw() // while循环语句
{
    ...
    // 放下label<check>
    Quaternary q_checklabel(Quaternary::LABEL, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND,
                            0, Quaternary::IMMEDIATE_OPERAND, checklabel);
...
void SyntaxAnalyzer::ForLoopStatement(int depth) throw() // for循环语句
{
    ...
    if (Token::IDENTIFIER != token_.type_) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be loop variable name after for\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::END) // 读到结尾或分号或END
        {
    ...
    {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  cannot be assigned in for loop\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::END) // 读到结尾或分号或END
        {
    ...
    // 检测to/downto
    if (Token::DOWNTO != token_.type_ && Token::TO != token_.type_) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be \"to\" or \"downto\" after variable assigning\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::END) // 读到结尾或分号或END
        {
    ...
    Quaternary q_init(Quaternary::ASG, init_attribute.operandtype_, init_attribute.value_,
    ...
    // 生成跳转到边界检查的JMP四元式
    Quaternary q_jmpcheck(Quaternary::JMP, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND, 0,
                          Quaternary::IMMEDIATE_OPERAND, checklabel);
    quaternarytable_.push_back(q_jmpcheck);
    // 生成循环变量递增/减的label<vary>
    Quaternary q_varylabel(Quaternary::LABEL, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND,
                           0, Quaternary::IMMEDIATE_OPERAND, varylabel);
    ...
    Quaternary q_vary(Token::TO == vary_token.type_ ? Quaternary::ADD : Quaternary::SUB,
                      ...
                      distance(tokentable_.begin(),
                               static_cast<TokenTable::const_iterator>(loopvar_iter)),
                      Quaternary::IMMEDIATE_OPERAND, 1, Quaternary::VARIABLE_OPERAND,
                      distance(tokentable_.begin(),
                               static_cast<TokenTable::const_iterator>(loopvar_iter)));
    ...
    Quaternary q_jmpvary(Quaternary::JMP, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND, 0,
...
void SyntaxAnalyzer::ContinueStatement(int depth) throw() // continue
{
    ...
    if (continue_label_.size() != 0) {
        Quaternary q_continue(Quaternary::JMP, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND,
                              0, Quaternary::IMMEDIATE_OPERAND, continue_label_.top());
        quaternarytable_.push_back(q_continue);
    } else {
        // 报错
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  no loop body found around \"continue\"\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::END) // 读到结尾或分号或END
        {
...
void SyntaxAnalyzer::BreakStatement(int depth) throw() // break
{
    ...
    if (break_label_.size() != 0) {
        Quaternary q_break(Quaternary::JMP, Quaternary::NIL_OPERAND, 0, Quaternary::NIL_OPERAND, 0,
                           Quaternary::IMMEDIATE_OPERAND, break_label_.top());
        quaternarytable_.push_back(q_break);
    } else {
        // 报错
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  no loop body found around \"break\"\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::END) // 读到结尾或分号或END
        {
...
// <过程调用语句> ::= '('[<实在参数表>]')'
void SyntaxAnalyzer::ProcedureCallStatement(
    const Token proc_token,
    const vector<TokenTableItem::DecorateType>& parameter_decorate_types,
    int depth) // 过程调用语句
{
    ...
    if (Token::LEFT_PAREN != token_.type_) {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  should be '(' to specify the argument\n";
        is_successful_ = false;
        while (token_.type_ != Token::NIL && token_.type_ != Token::SEMICOLON &&
               token_.type_ != Token::END) // 读到结尾或分号或END
        {
    ...
    {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  procedure does not take " << arg_decoratetypes.size() << " argument";
    ...
    for (int i = 0; i < static_cast<int>(parameter_decorate_types.size()); ++i) {
        ...
        if (parameter_decorate_types[i] < arg_decoratetypes[i]) {
            std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                      << "  cannot convert parameter " << i + 1 << " from "
                      << TokenTableItem::DecorateTypeString[parameter_decorate_types[i]] << " to "
                      << TokenTableItem::DecorateTypeString[arg_decoratetypes[i]] << "\n";
            is_successful_ = false;
...
// <函数调用语句> ::= '('[<实在参数表>]')'
void SyntaxAnalyzer::FunctionCallStatement(
    ...
    int depth) // 函数调用语句
{
    ...
    {
        std::cout << "line " << token_.lineNumber_ << ":  " << token_.toString()
                  << "  function does not take " << arg_decoratetypes.size() << " argument";
        if (arg_decoratetypes.size() > 1) {
            std::cout << "s\n";
        } else {
            std::cout << "\n";
