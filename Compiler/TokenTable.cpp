...
void TokenTable::AddVariableItem(Token variableIdentifier,
    ...
    TokenTableItem item(variableIdentifier.value_.identifier, TokenTableItem::VARIABLE,
                        decoratetype_, false, 0, level, addr_++);
    rows_.push_back(item);
}
...
int TokenTable::AddProcedureItem(Token procedureIdentifier, int level) throw() {
    ...
    TokenTableItem item(procedureIdentifier.value_.identifier, TokenTableItem::PROCEDURE,
                        TokenTableItem::VOID, false, 0, level, addr_);
    rows_.push_back(item);
    return rows_.size() - 1;
}
...
void TokenTable::AddParameterItem(Token parameterIdentifier,
    ...
    TokenTableItem item(parameterIdentifier.value_.identifier, TokenTableItem::PARAMETER,
                        decoratetype_, isref, 0, level, addr_++);
    rows_.push_back(item);
}
...
int TokenTable::GetParameterNum(int var_index) const throw() {
    do {
        --var_index;
    } while (var_index >= 0 && TokenTableItem::PROCEDURE != rows_[var_index].itemtype_ &&
