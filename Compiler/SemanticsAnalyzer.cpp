...
SemanticsAnalyzer::SemanticsAnalyzer(LexicalAnalyzer& lexical_analyzer) throw()
    : lexical_analyzer_(lexical_analyzer)
    , tokentable_()
    , token_()
    , level_(1)
    , is_successful_(true)
    , semantics_process_buffer_()
    , semantics_format_string_()
    , tokenbuffer_() {}
...
TokenTable SemanticsAnalyzer::GetTokenTable() const throw() { return tokentable_; }

string SemanticsAnalyzer::toString() const throw() { return semantics_process_buffer_.str(); }
...
#ifdef SEMANTICSDEBUG
        semantics_process_buffer_ << semantics_format_string_ << "  " << token_.toString()
                                  << std::endl;
#endif
        lexical_analyzer_.GetNextToken(token_);
    }
...
void SemanticsAnalyzer::TypeSpecification(size_t depth) throw() {
    ...
    if (TokenTableItem::ARRAY == itemtype_) // 若是数组
    {
        for (vector<Token>::const_iterator iter = tokenbuffer_.begin(); iter != tokenbuffer_.end();
             ++iter) {
            if (tokentable_.IsInLocalActiveScope(
...
void SemanticsAnalyzer::Statement(size_t depth) throw() {
    ...
    switch (token_.type_) {
        case Token::IDENTIFIER:
            ...
            {
                if (iter != tokentable_.end() && iter->itemtype_ != TokenTableItem::PROCEDURE &&
...
void SemanticsAnalyzer::AssigningStatement(const Token& idToken,
                                           TokenTable::iterator& iter,
                                           size_t depth) // 赋值语句
{
...
TokenTableItem::DecorateType SemanticsAnalyzer::Factor(size_t depth) throw() // 因子
{
    ...
    if (Token::IDENTIFIER == token_.type_) {
        ...
        if (Token::LEFT_BRACKET == token_.type_) {
            // 语义检查：是否为数组名
            if (iter != tokentable_.end() && iter->itemtype_ != TokenTableItem::ARRAY) {
                ErrorHandle(WRONGTYPE, (iter->name_ + " is not declared as an array").c_str());
                //return factor_attribute;
            }
            // 读入作为下标的表达式
        ...
        {
            // 语义检查：是否为函数
            if (iter != tokentable_.end() && iter->itemtype_ != TokenTableItem::FUNCTION) {
        ...
        } else // 单独一个标识符
        {
            ...
            if (iter != tokentable_.end() && iter->itemtype_ != TokenTableItem::VARIABLE &&
                iter->itemtype_ != TokenTableItem::PARAMETER &&
                iter->itemtype_ != TokenTableItem::CONST) {
                ErrorHandle(WRONGTYPE,
                            (iter->name_ + " should be varaible or parameter or constant").c_str());
                //return factor_attribute;
            }
        }
...
void SemanticsAnalyzer::CaseElement(size_t depth) throw() // 情况表元素
{
    ...
    do {
        ...
        if (Token::CONST_INTEGER != token_.type_ && Token::CONST_CHAR != token_.type_ &&
            ...
            ErrorHandle(WRONGTYPE,
                        (iter->name_ + " should be constant integer or character or constant variable after \"case\"")
                            .c_str());
            //return ;
        }
#ifdef SEMANTICSDEBUG
...
void SemanticsAnalyzer::ReadStatement(size_t depth) throw() // 读语句
{
    ...
    do {
        ...
        if (Token::LEFT_BRACKET == token_.type_) // 数组元素
        {
            ...
            // 读入右中括号的下一个单词
            lexical_analyzer_.GetNextToken(token_);
        } else if (iter != tokentable_.end() && iter->itemtype_ != TokenTableItem::VARIABLE &&
                   iter->itemtype_ != TokenTableItem::PARAMETER) {
            ErrorHandle(WRONGTYPE, (iter->name_ + " cannot be assigned").c_str());
        }
    } while (Token::COMMA == token_.type_);
...
void SemanticsAnalyzer::ForLoopStatement(size_t depth) throw() // for循环语句
{
    ...
    if (loopvar_iter != tokentable_.end() && loopvar_iter->itemtype_ != TokenTableItem::VARIABLE &&
...
// <过程/函数调用语句> ::= '('[<实在参数表>]')'
void SemanticsAnalyzer::ProcFuncCallStatement(const Token proc_token,
...
// <实在参数表> ::= <表达式>{,<表达式>}
void SemanticsAnalyzer::ArgumentList(const vector<ExpressionAttribute>& parameter_attributes,
    ...
    do {
        ...
        if (parameter_attributes.size() >= para_count &&
            ...
            std::cout << "warning: line " << token_.lineNumber_ << ":  " << token_.toString()
                      << "  cannot convert parameter " << para_count << " from "
                      << TokenTableItem::DecorateTypeString[argument_decoratetype] << " to "
                      << TokenTableItem::DecorateTypeString[parameter_attributes[para_count - 1].decoratetype_]
                      << "\n";
...
void SemanticsAnalyzer::ErrorHandle(ErrorType error_type, const char* errinfo) throw() {
    ...
    string errline = offset == string::npos
        ? ""
        : lexical_analyzer_.GetLine(token_.lineNumber_).substr(offset, count);
    switch (error_type) {
        ...
        case ARGUMENTNUMBERNOTMATCH: // 参数数量不匹配时，要读完整条语句并返回，因为参数数量不匹配，则无法继续判断参数类型，需要直接返回
            cout << "  line " << token_.lineNumber_ << ": " << errline
                 << "\terror token: " << token_.toString() << "\nerror info: "
                 << "argument number doesn't match the declaration\n";
            break;
