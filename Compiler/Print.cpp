...
void PrintOperand(Quaternary::AddressingMethod type,
                  int value,
                  const TokenTable& tokentable,
                  ostream& out,
                  int space_width) throw() {
...
void PrintQuaternaryVector(const vector<Quaternary>& quaternarytable,
                           const TokenTable& tokentable,
                           ostream& out) throw() {
    int i = 0;
...
bool PrintQuaternaryVector(const vector<Quaternary>& quaternarytable,
                           const TokenTable& tokentable,
...
void PrintQuaternaryComment(const vector<Quaternary>& quaternarytable,
                            const TokenTable& tokentable,
                            const vector<Quaternary>::const_iterator& c_iter,
                            ostream& out) throw() {
    // 先以注释形式输出待翻译的四元式
    out << "\n    ;";
    // 输出序号
...
void PrintStringVector(const vector<string>& stringtable, ostream& out) throw() {
    for (vector<string>::const_iterator iter = stringtable.begin(); iter != stringtable.end();
         ++iter) {
        //out.width(3);
        //out.setf(ios::right);
